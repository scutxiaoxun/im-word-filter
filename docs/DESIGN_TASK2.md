第一部分：完整的任务二 Markdown 文档内容（请复制全部）
markdown
# 任务二：游戏排行榜系统设计文档

> **设计原则**：积分为主键，时间为次键。积分高者绝对领先；积分相同时，最后更新时间越晚（越新）排名越高。所有设计均围绕 **高性能（写入微秒级、读取毫秒级）** 与 **工业级稳定性** 展开。
...
采用 **固定锚点方案（方案A）**，消除系统时钟回拨风险，且无需运维动态配置。
...
- **同分处理**：若 \( P_1 = P_2 \)，则 \( W_1 - W_2 = T_{update2} - T_{update1} \)。若玩家1更新更晚（\( T_{update1} > T_{update2} \)），则 \( W_1 > W_2 \)，排名靠前。
...
rank:game:{game_id}	ZSET	Member = user_id (字符串)
Score = 复合权重 
W
W (存储为 Double)	永久
...
public:
    ...
    // 定时器触发（建议间隔 1 秒）：将积攒的批次刷入 MySQL + Redis
    void FlushBatch() {
...
传统 ZREVRANGE key offset limit 在 offset 很大时（如 10 万），Redis 需遍历跳表节点，复杂度升至 
O
(
o
f
...
public:
    static std::vector<UserRankInfo> FetchNextPage(
        ...
        if (cursor.weight == 0) {
            ...
            std::string max_score = "(" + std::to_string(cursor.weight); // 排他性
...
O(1)	< 0.1 毫秒
后台批处理刷库（每秒）	批量 INSERT ... ON DUPLICATE KEY UPDATE	
O
(
K
)
...
MySQL 提交成功后，立即触发 Redis 异步 ZADD（使用非阻塞 redisAsyncCommand）。

若 Redis 写入失败，将 user_id 投递至本地重试队列，后台线程每隔 5 秒扫描重试。

定时对账任务：每日凌晨 3:00，扫描 MySQL 中 update_time 大于上次对账时间的增量用户，批量刷新 Redis。
